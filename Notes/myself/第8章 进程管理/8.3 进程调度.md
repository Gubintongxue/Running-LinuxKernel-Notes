...
- **用户空间API**：
  ...
  - `nice()`：用于调整进程的优先级。
  - `getpriority()`和`setpriority()`：获取和设置进程的`nice`值。
...
2. **虚拟时间计算**：CFS的关键是`vruntime`的计算，每个调度实体的数据结构`load_weight`包含权重和逆权重信息。
   ...
   - **虚拟时间计算公式**：
     ...
     vruntime=delta_exec×nice_0_weightweight\text{vruntime} = \text{delta\_exec} \times \frac{\text{nice\_0\_weight}}{\text{weight}}vruntime=delta_exec×weightnice_0_weight
...
- **选择下一个进程**：CFS调度器采用红黑树存储就绪进程，选择虚拟时间`vruntime`最小的进程（红黑树最左节点）作为下一个调度对象。
  ...
  ```
  c复制代码struct task_struct *pick_next_task_fair(struct rq *rq) {
...
```
c复制代码static void schedule(void) {
...
```
c复制代码#define switch_to(prev, next, last)  \
    do { /* ... */ } while(0)
```
